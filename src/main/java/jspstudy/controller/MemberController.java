...
public class MemberController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (command.equals("/member/memberJoinAction.do")) {
			...
			// 여러개의 값을 담을 때는 getParameterValues를 사용한다.
			...
			// IP 주소값을 뽑아내주는 메소드 getHostAddress
			...
			if (value == 1) {
				...
				// out.println("<script>alert('회원가입
				// 성공');location.href='"+request.getContextPath()+"/index.jsp'</script>");
			} else {
				response.sendRedirect(request.getContextPath() + "/member/memberJoin.do");
				// out.println("<script>alert('회원가입
				// 실패');location.href='./memberJoin.jsp'</script>");
			}

		} else if (command.equals("/member/memberJoin.do")) {
			System.out.println("test");
			// 회원가입 페이지로 들어오면 처리를 함
			RequestDispatcher rd = request.getRequestDispatcher("/member/MemberJoin.jsp"); // 클래스명 잘 확인할 것
			rd.forward(request, response);
			// forward란 내부적으로 이동하는 방식
